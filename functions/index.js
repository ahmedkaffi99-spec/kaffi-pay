...
// ══════════════════════════════════════════════════════════════════
// HTTP — ADMIN : Supprimer ordres à ID cassé (9+ chiffres, 12 juin)
// POST { _ak } — liste les ordres ; POST { _ak, confirm: true } — supprime
// ══════════════════════════════════════════════════════════════════
exports.adminCleanupBrokenIds = onRequest(
  { region: REGION, invoker: "public" },
  async (req, res) => {
    setCorsHeaders(res);
    if (req.method === "OPTIONS") { res.status(204).send(""); return; }
    ...
    // Ordres cassés : orderId commence par "0612" ET longueur >= 8
    const isBroken = (id) => typeof id === "string" && id.startsWith("0612") && id.length >= 8;

    const [depSnap, retSnap] = await Promise.all([
      db.collection("depot_orders").get(),
      db.collection("retrait_orders").get(),
    ]);

    const toDelete = [];
    for (const d of [...depSnap.docs, ...retSnap.docs]) {
      const data = d.data();
      if (isBroken(data.orderId)) {
        toDelete.push({ col: d.ref.parent.id, id: d.id, orderId: data.orderId, montant: data.montant, status: data.status });
      }
    }

    if (!confirm) {
      return res.json({ ok: true, dryRun: true, count: toDelete.length, orders: toDelete });
    }
    ...
    res.json({ ok: true, deleted: toDelete.length, orders: toDelete });
  }
);
