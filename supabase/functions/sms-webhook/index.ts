...
Deno.serve(async (req: Request) => {
  ...
  // Seule étape avant de répondre à MacroDroid : un insert Postgres local,
  // rapide et fiable. Tout le reste (Telegram, matching, MobCash) part en
  // arrière-plan via EdgeRuntime.waitUntil — sinon, avant ce changement, le
  // handler ne "return"ait qu'après avoir attendu Telegram ET tout le
  // pipeline de confirmation/crédit MobCash en bout de chaîne : sous une
  // rafale de plusieurs SMS dans la même seconde, Telegram peut rate-limiter
  // (429, silencieusement avalé par sendTelegram) et MobCash peut être lent,
  // ce qui retarde d'autant la réponse HTTP à MacroDroid. MacroDroid peut
  // alors considérer l'appel en échec (timeout de son action HTTP) sur un
  // SMS pourtant bien reçu et enregistré.
  ...
  (globalThis as any).EdgeRuntime?.waitUntil(
    traiterApresReponse(notifDoc as WaafiNotif, transferId, montant, numClient)
  );

  return response;
});

async function traiterApresReponse(
  notifDoc: WaafiNotif,
  ...
  try {
    ...
    console.error("sms-webhook telegram error:", e);
  }

  if (!transferId) {
    // Cas normal : le client paie avant de remplir le formulaire, ou le SMS
    // n'a pas de Transfer ID détectable. Rien de plus à faire ici.
    return;
  }
